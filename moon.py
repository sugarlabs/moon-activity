...
class MoonActivity(activity.Activity):
	...
	def __init__(self, handle):
		...
		self.toggleGridHandlerId = self.toggleGridButton.connect('clicked', self.toggleGridClicked)
		...
		self.toggleHemisphereHandlerId = self.toggleHemisphereButton.connect('clicked', self.toggleHemisphereClicked)
		...
		self.updateTextInformationView()
		self.updateMoonImageView()
	...
	def toggleHemisphereClicked(self, widget):
	...
	def blockViewButtonsDuringUpdate(self):
	...
	def unblockViewUpdateButtons(self):
	...
	def updateTextInformationView(self):
		"""Generate Moon data and update text based information view."""
		# Generate Moon data
		self.dataModel.updateMoonCalculations(time.time())

		# Update text information
		informationString = ""
		informationString += _("Today's Moon Information\n\n")
		informationString += _("Phase:\n%s\n\n") % (self.dataModel.moonPhaseName(self.dataModel.phaseOfMoon))
		...
		gobject.timeout_add(msToNextMinCusp, self.updateTextInformationView)

		# Stop this timer running
		return False

	def updateMoonImageView(self):
		...
		if self.hemisphereView == 'south':
			...
			if self.showGrid:
				...
				self.drawGrid(_("SNWE"))
		elif self.showGrid:
			# Draw grid for north hemi
			self.drawGrid(_("NSEW"))

		self.image.queue_draw()

		# Update the Moon image in another 5min
		self.updateMoonImageTimeout = gobject.timeout_add(300000, self.updateMoonImageView)
		
		# Delay before view buttons can be clicked again (blocked to stop repeat clicks)
		gobject.timeout_add(50, self.unblockViewUpdateButtons)

		# Stop this timer running
		return False

	def drawGrid(self, compassText):
...
class DataModel():
	...
	def updateMoonCalculations(self, theDate):
		"""Generate all Moon data ready for display."""
		SECONDS_PER_DAY = 86400.0
		lastNewMoonSeconds = self.lastNewMoonSecondsAtTime(theDate)
		nextNewMoonSeconds = self.nextNewMoonSecondsAtTime(theDate)
		lastFullMoonSeconds = self.lastFullMoonSecondsAtTime(theDate)
		nextFullMoonSeconds = self.nextFullMoonSecondsAtTime(theDate)
		lastQuarterMoonSeconds = self.lastQuarterMoonSecondsAtTime(theDate)
		nextQuarterMoonSeconds = self.nextQuarterMoonSecondsAtTime(theDate)
		...
		self.lunation = self.lunationAtTime(theDate)
		...
		self.nextNewMoonDate = theDate + nextNewMoonSeconds - self.correctForTzAndDst(theDate + nextNewMoonSeconds)
		self.daysUntilFullMoon = nextFullMoonSeconds / SECONDS_PER_DAY
		self.nextFullMoonDate = theDate + nextFullMoonSeconds - self.correctForTzAndDst(theDate + nextFullMoonSeconds)
		
		# Eclipse information
		self.nextLunarEclipseSeconds = self.nextLunarEclipseSecondsAtTime(theDate)
		self.nextSolarEclipseSeconds = self.nextSolarEclipseSecondsAtTime(theDate)
		self.lastLunarEclipseSeconds = self.lastLunarEclipseSecondsAtTime(theDate)
		self.daysUntilLunarEclipse = self.nextLunarEclipseSeconds / SECONDS_PER_DAY
		self.nextLunarEclipseDate = theDate + self.nextLunarEclipseSeconds - self.correctForTzAndDst(theDate + self.nextLunarEclipseSeconds)
		self.daysUntilSolarEclipse = self.nextSolarEclipseSeconds / SECONDS_PER_DAY
		self.nextSolarEclipseDate = theDate + self.nextSolarEclipseSeconds - self.correctForTzAndDst(theDate + self.nextSolarEclipseSeconds)
	...
	def correctForTzAndDst(self, dateSecOfEvent):
	...
	def moonPhaseName(self, phaseOfMoon):
	...
	def nextFullMoonSecondsAtTime(self, now):
	...
	def nextNewMoonSecondsAtTime(self, now):
	...
	def nextQuarterMoonSecondsAtTime(self, now):
	...
	def lastFullMoonSecondsAtTime(self, now):
	...
	def lastNewMoonSecondsAtTime(self, now):
	...
	def lastQuarterMoonSecondsAtTime(self, now):
	...
	def nextLunarEclipseSecondsAtTime(self, now):
	...
	def lastLunarEclipseSecondsAtTime(self, now):
	...
	def nextSolarEclipseSecondsAtTime(self, now):
